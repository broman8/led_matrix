...
void app_main(void) {
    ...
    // Draw Text (Static)
    led_matrix_clear();
    // Static text "Hello" starting at x=0
    led_matrix_draw_text("Hello", 0, 50, 0, 0, false, 0);
    led_matrix_refresh();
    vTaskDelay(pdMS_TO_TICKS(1000));

    // Draw Text (Scrolling)
    // Scrolls "World" from right to left with 50ms delay
    led_matrix_draw_text("World", 0, 0, 50, 0, true, 50);
}
```
